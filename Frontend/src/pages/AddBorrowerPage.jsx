...
export default function AddBorrowerPage() {
  ...
  const sendOtp = async () => {
    ...
    setload(true);
    try {
      await axios.post('http://localhost:3001/api/Otp/send-otp', { email: formData.borrowerEmail });
  ...
  const verifyOtp = async () => {
    ...
    setload(true);
    try {
      await axios.post('http://localhost:3001/api/Otp/verify-otp', { email: formData.borrowerEmail, otp: otpCode });
