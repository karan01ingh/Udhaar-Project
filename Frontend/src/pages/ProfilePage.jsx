...
export default function ProfilePage() {
  ...
  async function verifyCurrentPassword() {
    setload(true);
    try {
      await axios.post('http://localhost:3001/api/users/verify-password', {
        email: user.email,
  ...
  const onSubmitPassword = async (data) => {
    ...
    try {
      await axios.put("http://localhost:3001/api/users/updatePassword", { password: data.newPassword, userid: user.id }, { withCredentials: true });
